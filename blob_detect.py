...
for i in range(frame_count-4):
	...
	filt = cv2.GaussianBlur(mask_inv,(11,11),0)
	...
	masked_frame = cv2.bitwise_and(frame_grey, frame_grey, mask=bw_mask)
	# masked_frame[np.where((masked_frame==[0,0,0]).all(axis=2))] = [255,255,255]
